...
const Profile = () => {
  ...
  const handleRefundRequest = async (order) => {
    const reason = window.prompt('Why are you requesting a refund?', 'Item no longer needed');
    if (reason === null) return;

    setRefundingOrderId(order._id);
    try {
      const response = await fetch(`/api/orders/${order._id}/refund`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          Authorization: `Bearer ${user.token}`
        },
        body: JSON.stringify({ reason: reason.trim() || 'Customer requested a refund' })
      });
      const data = await response.json();
      if (!response.ok) throw new Error(data.message || 'Refund request failed');
      setOrders((current) => current.map((item) => item._id === data._id ? data : item));
      showToast('Refund request submitted for review', 'success');
    } catch (error) {
  ...
  return (
    <div style={containerStyle}>
      ...
      <h3 id="orders" style={{ color: '#f97316', marginBottom: '20px', fontSize: '1.5rem' }}>Order History</h3>
      {loading ? (
            ...
            <div key={order._id} style={{ background: '#09090b', padding: '20px', borderRadius: '12px', border: '1px solid #27272a', display: 'flex', flexWrap: 'wrap', justifyContent: 'space-between', alignItems: 'center', gap: '20px' }}>
              ...
              <div>
                ...
                {order.refundStatus === 'None' ? (
                  ...
                  <span className="refund-status">Refund: {order.refundStatus}</span>
                )}
              </div>
            </div>
          ))}
